...
class EulerAdaptiveScaleParamScore(Sampler):
    """
    ...
    def iter(
        ...
        # the denoiser returns directly a score
        score = self.denoiser(x_t, self.curr_t(curr_steps))
...
class SNREulerAdaptiveScaleParamScore(EulerAdaptiveScaleParamScore):
    """
    Uses 1st order Euler to integrate the SDE/ODE.
    local error: O(dt^2)
    """
...
class SNRHeunAdaptiveScaleParamScore(SNREulerAdaptiveScaleParamScore):
    """
    ...
    def _heun_correction_step(
        ...
        score = self.denoiser(x_t_next, self.curr_t(next_steps))
